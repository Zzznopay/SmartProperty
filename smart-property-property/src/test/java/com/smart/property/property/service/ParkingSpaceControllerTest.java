...
class ParkingSpaceControllerTest {
    ...
    @Test
    void testAddParking() {
        Community community = createCommunity("PK11");

        ParkingSpaceDTO dto = buildParking(community.getId(), "PK-T02");
        parkingSpaceService.createParking(dto, 1L, "test");

        ParkingSpaceVO saved = findByParkingNo(community.getId(), "PK-T02");
        assertNotNull(saved);
    ...
    void testSaleParking() {
        Community community = createCommunity("PK13");
        Owner owner = createOwner("PK13-O");

        ParkingSpaceDTO dto = buildParking(community.getId(), "PK-S02");
        parkingSpaceService.createParking(dto, 1L, "test");
        Long id = findByParkingNo(community.getId(), "PK-S02").getId();
    ...
    void testRentParking() {
        Community community = createCommunity("PK14");
        Tenant tenant = createTenant("PK14-T");

        ParkingSpaceDTO dto = buildParking(community.getId(), "PK-R02");
        parkingSpaceService.createParking(dto, 1L, "test");
        Long id = findByParkingNo(community.getId(), "PK-R02").getId();
