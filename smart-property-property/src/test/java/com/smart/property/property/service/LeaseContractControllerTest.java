...
@SpringBootTest
@Transactional
class LeaseContractControllerTest {
    ...
    void testAddContract() {
        Room room = createRoom("LC11");
        Tenant tenant = createTenant("LC11-T");

        LeaseContractDTO dto = buildContract("LC-A1", room.getId(), tenant.getId(), 1);
        leaseContractService.createContract(dto, 1L, "test");

        LeaseContractVO saved = findByContractNo("LC-A1");
        assertNotNull(saved);
        LeaseContractVO loaded = leaseContractService.getContractById(saved.getId(), 1L);
        assertEquals("LC-A1", loaded.getContractNo());
        assertEquals(1, loaded.getStatus());
    }

    @Test
    void testGetContractPage() {
        Room room = createRoom("LC12");
        Tenant tenant = createTenant("LC12-T");

        LeaseContractDTO dto = buildContract("LC-P2", room.getId(), tenant.getId(), 1);
        leaseContractService.createContract(dto, 1L, "test");
    ...
    void testTerminateContract() {
        Room room = createRoom("LC13");
        Tenant tenant = createTenant("LC13-T");

        // 状态2：生效
        LeaseContractDTO dto = buildContract("LC-T2", room.getId(), tenant.getId(), 2);
        leaseContractService.createContract(dto, 1L, "test");
        Long id = findByContractNo("LC-T2").getId();
    ...
    void testGetExpiringContracts() {
        Room room = createRoom("LC14");
        Tenant tenant = createTenant("LC14-T");

        LeaseContractDTO dto = buildContract("LC-E2", room.getId(), tenant.getId(), 2);
