...
class NoticeServiceTest {
    ...
    private NoticeVO createNotice(String title, Integer status) {
        ...
        dto.setNoticeContent("公告内容");
        dto.setNoticeType(1);
        dto.setStatus(status);
        noticeService.createNotice(dto, 1L, "admin");
    ...
    @Test
    void testSaveNotice() {
        NoticeVO created = createNotice("NOTICE-T1-01", 1);

        assertNotNull(created.getId());
        assertEquals("NOTICE-T1-01", created.getNoticeTitle());
    }

    @Test
    void testGetNoticePage() {
        createNotice("NOTICE-T1-02", 1);

        PageQuery query = new PageQuery();
        query.setPageNum(1);
        query.setPageSize(10);

        PageResult<NoticeVO> result = noticeService.getNoticePage(query, 1L, null);
    ...
    void testPublishNotice() {
        NoticeVO created = createNotice("NOTICE-T1-03", 1);
    ...
    void testRevokeNotice() {
        NoticeVO created = createNotice("NOTICE-T1-04", 1);
    ...
    @Test
    void testMarkRead() {
        NoticeVO created = createNotice("NOTICE-T1-05", 2);
